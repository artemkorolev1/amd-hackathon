#!/usr/bin/env python3
"""Evaluate 6 prompt strategies across GGUF models on 94 math questions on GPU.
Each model runs as a SEPARATE subprocess to avoid OOM.
Fixed: regex escaping in worker code is correct."""
...
def _write_worker(model_path, tmp_path):
    """Write a worker script to a temp file. Uses explicit write to avoid interpolation issues."""
    code = f'''#!/usr/bin/env python3
import json, re, time, os, gc, sys
os.environ['PYTHONUNBUFFERED'] = '1'
DATA_PATH = "/home/artem/dev/amd-hackathon/data/eval/math_combined_80.json"
PROMPT_STRATEGIES = [
    {{"index": 0, "label": "empty", "system_prompt": ""}},
...
def eval_model_subprocess(model_name, model_path):
    ...
    worker_code = _write_worker(model_path, tmp_path)
