...
@dataclass
class Stage0Result:
    """Result of Stage 0 pre-filtering."""
...
# ============================================================
# TIER 1 — Route to category (bypass Stage 1-2, go to Stage 3)
# ============================================================
...
def stage0(prompt: str) -> Stage0Result:
    """
    Run Stage 0 pre-filters in priority order.

    Returns a Stage0Result directing what should happen next.
    """
    text = prompt.strip()
    if not text:
        return Stage0Result(action="bypass", direct_answer="")
    ...
    # ── Tier 1: Route to category (bypass Stage 1-2) ────────
    ...
    # ── Nothing matched ─────────────────────────────────────
    return Stage0Result(action="continue", category=None, flags={})
